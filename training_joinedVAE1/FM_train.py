...
# ==== Training Loop ====
def train(model, dataloader, optimizer, device, log_interval=10):
    model.train()
    total_loss = 0
    total_recon = 0
    total_kl = 0
    ...
    for step, batch in enumerate(dataloader):
        ...
        loss, recon_loss, kl_div, latent_consistency_loss = loss_fn(preds, targets)
        loss.backward()
        optimizer.step()
...
def TrainFMmodel(
    ...
    with open(log_path, 'w') as f_log, redirect_stdout(f_log):
        ...
        for epoch in range(n_epoch):
            ...
            train_loss, train_recon, train_kl, train_latent = train(model, train_loader, optimizer, device, log_interval=train_log_interval)
            val_loss, val_recon, val_kl, val_latent = validate(model, val_loader, device)
