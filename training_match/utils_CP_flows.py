...
def segment_image(
    img_path,
    CP_model_name = None,
    CP_model_path = "/home/ddc/CP_model_zoo/models/",
    use_GPU = True,
    savedir = "./",
    channels = [0, 0],
    ...
    path_CP_masks = f"{savedir}/{base_name}_CP_masks.pkl"
    path_CP_flows = f"{savedir}/{base_name}_CP_flows.pkl"

    # check if the CP output already exists
    if os.path.exists(path_CP_masks) and os.path.exists(path_CP_flows):
        print(f"- Load existing Cellpose outputs from {savedir}")
        with open(path_CP_masks, "rb") as f:
            masks = pickle.load(f)
        with open(path_CP_flows, "rb") as f:
            flows = pickle.load(f)
    else: 
        ...
        # save CP outputs
        print(f"- Save Cellpose outputs to {savedir}")
        with open(path_CP_masks, "wb") as f:
            pickle.dump(masks, f)
        with open(path_CP_flows, "wb") as f:
            pickle.dump(flows, f)

        # save CP masks as image for Fiji
        save_CP_mask_image_for_Fiji(masks[0], base_name, savedir)
...
def save_CP_mask_image_for_Fiji(masks, img_name, savedir):
...
def compare_2_slides(
    ...
    cp_output1 = segment_image(path_img1, CP_model_name_1, savedir=dir_save_cellpose_masks)
    mask1, p1, _ = cp_output1
    cp_output2 = segment_image(path_img2, CP_model_name_2, savedir=dir_save_cellpose_masks)
    mask2, p2, _ = cp_output2
    
    labels1 = filter_ROIs(cp_output1, size_crop)
    labels2 = filter_ROIs(cp_output2, size_crop)
    ...
    if plot_annotated_pairs:
        ...
        list_roi1 = [r for r in df_0.ROI_I1.tolist() if r in labels1]
        list_roi2 = [r for r in df_0.ROI_I2.tolist() if r in labels2]
            ...
            if plot_annotated_pairs:
                if ax_idx==0:
                    if region.label in list_roi1:
                        ax.text(x, y, str(region.label), color='red', fontsize=8,
                            ha='center', va='center', fontweight='bold')
                    elif (region.label not in list_roi1) & (region.label in labels1):
                ...
                if ax_idx==1:
                    if region.label in list_roi2:
                        ax.text(x, y, str(region.label), color='red', fontsize=8,
                            ha='center', va='center', fontweight='bold')
                    elif (region.label not in list_roi2) & (region.label in labels2):
                        ax.text(x, y, str(region.label), color='white', fontsize=11,
                            ha='center', va='center', fontweight='bold')
                    else:
                        ax.text(x, y, str(region.label), color='gray', fontsize=6,
